...
mes_pts_reg_geo <- point_prelevement %>% 
  filter(pop_id %in% mes_pts) %>% 
...
ope_donnees_env <- operation_description_peche %>% 
...
ope_donnees_env <- ope_donnees_env %>%
  left_join(y = ope_granu) %>%
...
reg_donnees_env <- ope_donnees_env %>% 
...
```{r}
reg_pp <- probabilite_presence_ipr %>%
