...
# objet classe sf de points sélectionnés
mes_reg_pop_geo <- point_prelevement %>% 
  filter(pop_id %in% mes_pts) %>% 
...
# reseau <- tod::wfs_sandre(
#   url_wfs = "https://services.sandre.eaufrance.fr/geo/sandre",
#   couche = "eth")
```
...
# reg_dept_dens_num <- reg_ope_capt %>%
#   group_by(dept, annee, esp_code_alternatif) %>%
#     summarise(densite_qd_pres_med = median(densite, na.rm = FALSE),
#               densite_qd_pres_moy = mean(densite, na.rm = FALSE),
#               densite_yc_abs_moy = 1000 * sum(effectif, na.rm = FALSE) / sum(ope_surface_calculee,
#                                                                             na.rm = FALSE)
#               ) %>%
#   ungroup()
...
# reg_dept_dens_num <- reg_dept_dens_num %>%
#   left_join(y = ref_espece %>%
#               select(esp_code_alternatif,
#                      esp_nom_commun))
```
...
```{r}
# reg_dept_dens_num_poissons_loc <- reg_dept_dens_num %>%
...
#   mutate(esp_nom_commun2 = paste0(esp_nom_commun,
#                                  " (",
#                                  esp_code_alternatif,
#                                  ")"))
...
```{r}
reg_ope_metrique <- operation_ipr %>% 
