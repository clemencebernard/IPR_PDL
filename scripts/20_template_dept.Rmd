...
- RRP : Le Réseau de Référence Pérenne est mis en œuvre pour conforter la connaissance des conditions de référence qui servent à définir le bon état écologique de la DCE et suivre leur évolution temporelle, notamment sous les effets du changement climatique.

- RHP : Le Réseau Hydrobiologique et Piscicole, mis en place en 1990, vise à disposer chaque année d'un état des peuplements piscicoles, à caractériser les variations interannuelles et les tendances, à évaluer les conséquences des pression anthropiques des évènements naturels exceptionnels. Il permet également une veille écologique des espèces.


Dans une première partie est exposée une synthèse des résultats à l’échelle du département.

Cette synthèse expose l’état des peuplements ichtyologiques à travers l’indice poissons Rivière (IPR, Oberdorff *et al.* 2001) et différentes métriques de la qualité du peuplement (richesse en espèces, occurences, abundance, repartition en classes de taille. Ces données sont également recontextualisées dans des suivis chronologiques de long terme (à partir de 1997) afin de permettre au lecteur de prendre du recul sur les variabilités interannuelles et de visualiser les éventuelles tendances temporelles. 
...
mes_pts_dept_geo <- mes_reg_pop_geo %>% 
...
Les graphiques ci-dessous présentent l’évolution des paramètres abiotiques environnementaux qui caractérisent la station. La ligne rouge est la médiane de l’ensemble des données des stations du département qui ont été saisies depuis 1997.
...
Ce graphique reprend les notes IPR calculées depuis 2007, date de début du RCS et stabilisation des protocoles d'éhantillonnage. 

Les points correspondent à la note des IPR de la station par année. 


```{r, fig.width = 10, fig.height = reg_ope_ipr %>% filter(dept == params$mon_dept) %>% pull(pop_libelle) %>% unique %>% length %>% `/`(4) %>% ceiling()%>% `*`(2.5)%>% `+`(1)}
...
ATENTION : Pour une meilleure lisibilité, l’échelle de l’ordonnée est en log. Ainsi, les données ne sont pas écrasées lorsqu’il y a beaucoup ou peu d’individus. 

Les lignes de couleurs correspondent à la densité spécifique moyenne des départements bretons. 

Sur certaines courbes, on observe un pic tous les deux ans. Celui-ci est dû au RCS qui est réalisé un an sur deux. 

L’intérêt de mettre en évidence la densité spécifique moyenne interdépartementale permet de comparer les dynamiques d’évolutions pour chacune des espèces. 
...
map(.x = mes_especes,
  ...
  map(.f = function(gg) {gg + labs(x = "Longueur (cm)")})
```
