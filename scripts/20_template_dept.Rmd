...
# title: |
#   ![](logo_OFB_v2.png){width=1in, align="right"}  
...
#     \includegraphics[width=2in,height=2in]{logo_OFB_v2.png}\LARGE\\}
...
knitr::opts_chunk$set(echo = FALSE, warning = FALSE, message = FALSE, fig.pos = "H", out.extra = "", eval.after = "fig.cap")
```
...
Ce rapport présente l’ensemble des pêches scientifiques réalisées dans le département au titre des réseaux de suivi de l’état des milieux, du `r premiere_date` au `r derniere_date`. Les réseaux couverts sont : 

- RCS : Le Réseau de Contrôle de Surveillance vise à évaluer à large échelle l’état écologique et à suivre les changements à moyen terme de cet état, notamment en lien avec les mesures de gestion des milieux prévues par la DCE.
...
Une première partie est consacrée à la synthèse des résultats à l’échelle du département.

Cette synthèse expose l’état des peuplements ichtyologiques à travers l’Indice Poissons Rivière (IPR, Oberdorff *et al.* 2001) et différentes métriques de la qualité du peuplement (richesse en espèces, occurrence, abondance, répartition en classes de taille). Ces données sont également contextualisées dans des suivis chronologiques de long terme (à partir de 1997) afin de permettre au lecteur de prendre le recul nécessaire sur les variabilités interannuelles pour mieux visualiser les éventuelles tendances temporelles. 

Une deuxième partie présente les fiches de résultats bruts et élaborés à l’échelle de la station. Les données sont représentées graphiquement pour visualiser l’évolution de la classe de qualité IPR, de ses métriques, et un panel de variables écologiques facilitant l’interprétation.

```{r, fig.width = 10, fig.height = 10, fig.cap = paste0("Carte du département et stations réseaux. Pour chaque station la couleur correspond à la dernière année disponible. Les stations réseaux qui ne sont prospectées qu'une année sur deux sont toutes portées sur la carte, mais celles qui n'ont été prospectées qu'en ", as.integer(params$mon_annee) - 1, " sont représentées par des points plus petits.")}
...
- Granulométrie : L’axe des ordonnées varie de 0 à 12. Chacun des chiffres correspond à une référence de granulométrie (voir légende ci-dessous).
...
- Largeur mouillée et longueur station sont exprimées en m.
- Profondeur : en cm.
- Température : en °C.

```{r, fig.width = 9, dpi = 300, fig.cap = "Evolution interannuelle des paramètres abiotiques environnementaux (valeur médiane calculée sur l'ensemble des stations du département). La ligne rouge est la médiane interannuelle depuis 2007. Les données antérieures à 2007 sont représentées en pointillés car elles sont moins fiables."}
...
Ces graphiques présentent l’évolution des paramètres de réglage électrique ainsi que la conductivité. Ils se lisent comme les précédents.


```{r, fig.width=9, dpi=300, fig.cap = "Evolution interannuelle des paramètres électriques lors des échantillonnages. Conductivité : Capacité de l’eau de la station à conduire le courant électrique (charge en ions), en µS/cm. Les paramètres du courant délivré sont l'intensité (A), la puissance (kVA) et la tension (V)."}
dept_donnees_env %>% 
  filter((variable %in% c("Conductivité", "Intensité", "Tension", "Puissance"))) %>% 
  ggplotExtra::gg_lattice_line(
  var_x = annee,
...
Les données sont présentées depuis 2007, année de stabilisation des réseaux DCE et des protocoles d’échantillonnage.

Le calcul de la note IPR est la somme de 7 métriques qui évaluent divers traits écologiques de la communauté de poissons : 3 métriques d’occurrence (présence-absence des espèces) et 4 métriques d’abondance (densités d’individus).
...
Les graphiques ci-dessous se lisent comme le précédent.
...
```{r, fig.width = 8, fig.height = 6, fig.cap = "Evolution interannuelle des métriques IPR médianes sur le département. Les seuils des classes de qualité sont ceux de l'IPR agrégé divisés par 7."}
...
```{r, fig.width = 10, fig.height = reg_ope_ipr %>% filter(dept == params$mon_dept) %>% pull(pop_libelle) %>% unique %>% length %>% `/`(4) %>% ceiling() %>% `*`(2.1)%>% `+`(1), fig.cap = "Evolution interannuelle de l'IPR sur les stations du département."}
...
### Occurence par département

```{r, fig.height = 8, fig.width = 6, dpi = 300, fig.cap = "Comparaison entre les cortèges d'espèces des départements de la région."}
...
La taille des points indique le pourcentage d’occurrence d'une espèce dans chaque département, c'est à dire le nombre de stations où l'espèce est observée sur le total de stations prospectées dans le département. Les espèces sont ordonées par nombre de départements de présence, puis par nombre total d’occurrences.

Exemple : Si, dans le Morbihan, le Chevaine a été capturé sur 8 des 16 stations prospectées en `r  params$mon_annee`. Son taux d’occurrence dans ce département est donc de 50%.

Les écrevisses apparaissent en bleu.

### Tendances des populations par département

Les lignes de couleur indiquent, pour chaque département, la densité surfacique moyenne par espèce en fonction de l'année.

NB : Pour une meilleure lisibilité, l’échelle de l’ordonnée est en log. Ainsi, les données ne sont pas écrasées lorsqu’il y a beaucoup ou peu d’individus. 

Sur certaines courbes, on observe un pic tous les deux ans. Celui-ci est dû au RCS qui est réalisé un an sur deux. 
...
```{r, fig.width = 10, fig.height = fig_haut_reg_ecr, fig.cap = "Evolution interannuelle des densités en écrevisses, comparaison entre les départements de la région."}
reg_dept_capt %>% 
...
```{r, fig.width = 10, fig.height = fig_haut_reg_poissons, fig.cap = "Evolution interannuelle des densités en poissons, comparaison entre les départements de la région."}
reg_dept_capt %>% 
...
Le premier diagramme indique le nombre de captures par espèce dans le département pour l’année `r  params$mon_annee`. Le second s'inscrit dans une perspective de fonctionnement trophique, et montre la biomasse cumulée de ces captures par espèce. 
...
```{r, fig.cap = paste0("Effectif capturé par espèce dans le ", params$mon_dept, " en ", params$mon_annee,".")}
reg_dept_capt %>%
  filter(dept == params$mon_dept,
...
```{r, fig.cap = paste0("Biomasse capturé par espèce dans le ", params$mon_dept, " en ", params$mon_annee,".")}
reg_dept_capt %>%
  filter(dept == params$mon_dept,
         annee == params$mon_annee,
         poids > 0) %>% 
...
```{r, fig.height=2.5, fig.width=5, fig.cap = "Nombre d'espèces capturées chaque année dans le département, selon leur statut."}
reg_rs %>% 
  filter(dept == params$mon_dept) %>% 
  ggplot(aes(x = annee,
             y = rs,
...
Seules les 10 espèces les plus abondantes sont représentées. Pour calculer des classes de tailles représentatives et discerner les générations, il faut un jeu de donnée suffisamment important.  
