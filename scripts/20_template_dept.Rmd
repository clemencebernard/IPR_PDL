...
params:
  mon_dept: "11"
  mon_annee: "2022"
  afficher_graph_pops_depts: TRUE
...
Ce rapport présente l’ensemble des pêches scientifiques réalisées dans le département au titre des réseaux de suivi de l’état des milieux, du `r premiere_date` au `r derniere_date`. Les données sont extraites de la base ASPE, administrée par l'OFB [@irz_long-term_2022]. Les réseaux couverts sont : 
...
Une deuxième partie présente les fiches de résultats bruts et élaborés à l’échelle de la station. Pour une meilleure compréhension, les données sont mises sous la forme de graphiques permettant de visualiser l’évolution de la classe de qualité IPR, de ses métriques, et un panel de variables écologiques facilitant l’interprétation.

```{r fig1, fig.width = 10, fig.height = 10, fig.cap = paste0("Carte du département et stations réseaux. Pour chaque station la couleur correspond à la dernière année disponible. Les stations réseaux qui ne sont prospectées qu'une année sur deux sont toutes portées sur la carte, mais celles qui n'ont été prospectées qu'en ", as.integer(params$mon_annee) - 1, " sont représentées par des points plus petits.")}
...
La Figure \@ref(fig:fig2) présente l’évolution temporelle des paramètres abiotiques environnementaux qui caractérisent la station. Les paramètres sont les suivants :
...
```{r, fig.width = 10, fig.height = fig_haut_reg_poissons, fig.cap = "Evolution interannuelle des densités en poissons dans le département. Cette figure permet mieux que la précédente de distinguer ce qui se passe dans le département car l'échelle des ordonnées est mieux adaptée."}
g <- reg_dept_capt %>% 
  filter(esp_nom_commun %in% reg_poissons,
...
```{r, eval = params$afficher_graph_distri_tailles, fig.height = 8, results = 'hide'}
map(.x = mes_especes,
    .f = gg_dyn_esp,
    df = longueurs,
