...
# dept_ipr %>%
#   group_by(annee) %>%
#   summarise(ipr25 = quantile(ipr, 0.25),
...
Interprétation du graphique ci-dessous : En pointillés c’est la médiane annuelle de l’IPR. La zone grisée les quantiles 25% et 75% (homologues de la « boite » dans les boîtes à moustache). En bleu c’est la tendance linéaire pour le département et en marron la médiane régionale.
...
# toutes les combinatsons espèce x operation
reg_sp_combin_sp_ope <- expand.grid(ope_id = unique(reg_sp$ope_id),
                                    esp_code_alternatif = unique(reg_sp$esp_code_alternatif))

# complétude du tableau avec les absences et calcul des densités
reg_sp2 <- reg_sp_combin_sp_ope %>% 
  left_join(reg_sp) %>% 
  mef_ajouter_surf_calc() %>% 
  mutate(effectif = replace_na(effectif, 0), 
         densite = effectif / ope_surface_calculee)

reg_densites_an <- reg_sp2 %>% 
