---
title: "Synthèse des pêches d'échantillonnage en rivière, année `r params$mon_annee`"
...
#   mutate(
#          poids_retenu = case_when(
#            mei_poids > 0 ~ mei_poids,
#            mei_poids_estime > 0 ~ mei_poids_estime,
#            TRUE ~ poids_tp#,
#           # TRUE ~ NA_real
#          )) 
# 
...
rs <- reg_data %>% 
  ...
  group_by(annee, introduite) %>% 
  summarise(rs = n_distinct(esp_code_alternatif)) %>% 
  ungroup() %>% 
  mutate(espece = ifelse(test = introduite == TRUE, "Introduite", "Autochtone"))
...
pp <- probabilite_presence_ipr %>%
  filter(ppi_opi_ope_id %in% mes_operations) %>%
