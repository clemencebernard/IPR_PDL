...
n_occ <- reg_esp %>% 
  group_by(dept, esp_code_alternatif) %>% 
    tally()

pc_occ <- n_occ %>% 
  left_join(y = nb_pop) %>% 
  mutate(pc_occ = n / n_pop * 100,
         #taille_pt = pc_occ ^ 0.5,
         dept_fct = as.factor(dept),
         dept_fct = fct_relevel(dept_fct, "35", "56", "29", "22")#,
         # espece = fct_infreq(esp_code_alternatif),
         # espece = fct_relevel(espece, rev)
         )

n_dept_par_esp <- reg_esp %>% 
  group_by(esp_code_alternatif) %>% 
  summarise(n_dept = n_distinct(dept))


pc_occ <- pc_occ %>% 
  left_join(n_dept_par_esp) %>% 
  mutate(espece = fct_reorder(esp_code_alternatif, n_dept))
